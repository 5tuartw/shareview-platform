...
export interface KeywordBrandSplitsServiceResult {
  data: unknown
  status: number
  activityDetails?: {
...
export async function getRetailerKeywordBrandSplits(
  retailerId: string,
  searchParams: URLSearchParams
): Promise<KeywordBrandSplitsServiceResult> {
  ...
  if (!tableResult.rows[0]?.exists) {
    return {
      data: { error: 'Brand Splits snapshot tables are not available yet' },
      status: 404,
    }
  }
  ...
  if (!isBrandSplitScope(scopeParam)) {
    return { data: { error: 'Invalid scope parameter' }, status: 400 }
  }
  ...
  if (!classification) {
    return { data: { error: 'Invalid classification parameter' }, status: 400 }
  }
  ...
  if (!period) {
    return {
      data: { error: 'No Brand Splits snapshot data available for this retailer' },
      status: 404,
    }
  }
  ...
  if (snapshotResult.rows.length === 0) {
    ...
    return {
      data: { error: 'No Brand Splits snapshot data available for this period', nearest_before, nearest_after },
      status: 404,
    }
  }
