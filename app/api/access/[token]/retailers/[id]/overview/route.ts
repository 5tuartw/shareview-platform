...
export async function GET(
  ...
  try {
    ...
    const result = await getRetailerOverview(retailerId, searchParams)
    return NextResponse.json(result.data, { status: result.status })
  } catch (error) {
    console.error('Error in access token overview proxy:', error)
    return NextResponse.json(
      { error: 'Failed to fetch overview metrics', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    )
  }
}
