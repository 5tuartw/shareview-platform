...
export async function GET(
  ...
  try {
    ...
    if (result.rows.length > 0) {
      const config: RetailerConfigResponse = result.rows[0];
  ...
  try {
    ...
    const result = await query(
      ...
      [
        ...
        visible_metrics || VALID_METRICS,
        keyword_filters || [],
        JSON.stringify(features_enabled || { insights: true, competitor_comparison: true, market_insights: true }),
        parseInt(session.user.id),
      ]
    );

    const updatedConfig: RetailerConfigResponse = result.rows[0];
